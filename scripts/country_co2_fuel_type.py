...
def main():
    ...
    coal_gcp = pd.melt(
        coal_gcp, id_vars=["Year"], var_name=["Country"], value_name="Coal"
    )
    flaring_gcp = pd.melt(
        flaring_gcp, id_vars=["Year"], var_name=["Country"], value_name="Flaring"
    )
    cement_gcp = pd.melt(
        cement_gcp, id_vars=["Year"], var_name=["Country"], value_name="Cement"
    )

    emissions_gcp = (
        gas_gcp.merge(oil_gcp, on=["Year", "Country"])
    ...
    emissions_gcp = (
        emissions_gcp.merge(country_gcp, on="Country")
    ...
    emissions_cdiac = emissions_cdiac.rename(
        columns={
            "Nation": "Country",
    ...
    emissions_cdiac = emissions_cdiac.drop(
        columns={
    ...
    other_columns = sorted(
        [col for col in combined.columns if col not in ["Country", "Year"]]
    )
    ...
    for col in per_capita_cols:
        combined[f"{col} (per capita)"] = (
            combined[col] / combined["Population"] * 1000000
        )
    ...
    # Save to CSV file
    combined.to_csv(os.path.join(GRAPHER_DIR, "co2_fuel.csv"), index=False)
