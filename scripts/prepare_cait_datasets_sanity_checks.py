...
OUTPUT_FILE = os.path.join(
    CURRENT_DIR,
    "sanity_checks",
    f"cait_datasets_sanity_checks_<DATASET_NAME>_{DATE_TAG}.html",
)
...
DATASET_OLD_FILES = {
    "emissions_all_ghg": OWID_DATASETS_URL
    + "GHG%20Emissions%20by%20Country%20and%20Sector%20(CAIT%2C%202020)/GHG%20"
    "Emissions%20by%20Country%20and%20Sector%20(CAIT%2C%202020).csv",
    "emissions_co2": OWID_DATASETS_URL
    + "CO2%20emissions%20by%20sector%20(CAIT%2C%202020)/CO2%20emissions%20by%20"
    "sector%20(CAIT%2C%202020).csv",
    "emissions_ch4": OWID_DATASETS_URL
    + "Methane%20emissions%20by%20sector%20(CAIT%2C%202020)/Methane%20emissions%20"
    "by%20sector%20(CAIT%2C%202020).csv",
    "emissions_n2o": OWID_DATASETS_URL
    + "Nitrous%20oxide%20emissions%20by%20sector%20(CAIT%2C%202020)/Nitrous%20oxide"
...
NAME_CO2_NEW = {
    col: NAME_ALL_NEW[col]
    for col in NAME_ALL_NEW
    if col
    not in ["Agriculture", "Agriculture (per capita)", "Waste", "Waste (per capita)"]
}
...
NAME_CH4_NEW = {
    ...
    not in [
        ...
        "International aviation and shipping (per capita)",
...
def load_population(name_population=NAME_POPULATION, name=NAME):
    ...
    population_dataset_url = (
        "https://github.com/owid/owid-datasets/raw/master/datasets/Population%20(Gapminder%2C%20"
...
def check_that_energy_is_well_calculated(
    data,
    min_relevant_value=1,
    min_relevant_error=1,
    name=NAME,
    ...
    relevant_columns = [
        ...
        name["manufacturing_and_construction"],
        name["other_fuel_combustion"],
        name["transport"],
    ]
    energy_estimated = data[
        [col for col in relevant_columns if col in data.columns]
    ].sum(axis=1)
    comparison = pd.DataFrame(
        {"estimated": energy_estimated, "reported": data[name["energy"]]}
    )
    select_relevant = (comparison["estimated"] > min_relevant_value) | (
        comparison["reported"] > min_relevant_value
    )
    comparison[error_name] = error_function(
        old=comparison[select_relevant]["reported"],
        new=comparison[select_relevant]["estimated"],
    )
    warnings = comparison[comparison[error_name] > min_relevant_error].reset_index(
        drop=True
    )
...
def main(
    dataset_name,
    output_file,
    dataset_old_files=DATASET_OLD_FILES,
    dataset_new_files=DATASET_NEW_FILES,
    name=NAME,
    name_old=NAME_OLD,
    name_new=NAME_NEW,
    ranges=RANGES,
    ...
    old = sanity_checks.rename_columns(
        data=old_raw,
        entities_to_rename_in_columns=list(name_old[dataset_name]),
        name_dataset=name_old[dataset_name],
        name_default=name,
    )
    new = sanity_checks.rename_columns(
        data=new_raw,
        entities_to_rename_in_columns=list(name_new[dataset_name]),
        name_dataset=name_new[dataset_name],
        name_default=name,
    )
    ...
    if ignore_lucf:
        ...
        warnings_comparing_datasets = warnings_comparing_datasets[
            (
                ...
                == "check_that_values_did_not_change_abruptly_from_old_to_new_dataset"
            )
