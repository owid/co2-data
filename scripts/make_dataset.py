...
def add_iso_codes(dataframe):
    iso_codes = pd.read_csv(
        os.path.join(INPUT_DIR, "shared/iso3166_1_alpha_3_codes.csv")
    )
...
def main():
    ...
    ghg_emissions = ghg_emissions.rename(
        columns={
    ...
    combined = (
        combined.merge(
            primary_energy, on=["Year", "Country"], how="left", validate="1:1"
        )
    ...
    combined.to_csv(os.path.join(OUTPUT_DIR, "owid-co2-data.csv"), index=False)
    combined.to_excel(os.path.join(OUTPUT_DIR, "owid-co2-data.xlsx"), index=False)
