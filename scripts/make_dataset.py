...
def prepare_data(table: catalog.Table) -> pd.DataFrame:
    ...
    df = df[first_columns + [column for column in sorted(df.columns) if column not in first_columns]]
...
def prepare_codebook(table: catalog.Table) -> pd.DataFrame:
    ...
    table["iso_code"].metadata.sources = [catalog.meta.Source(name="International Organization for Standardization")]
    table["population"].metadata.description = "Population of each country or region."
    table["population"].metadata.sources = [catalog.meta.Source(
