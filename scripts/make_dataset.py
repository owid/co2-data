...
def prepare_codebook(table: catalog.Table) -> pd.DataFrame:
    ...
    for column in table.columns:
        ...
        for source in table[column].metadata.sources:
            ...
            ############################################################################################################
            # TODO: Remove this temporary solution once we combine multiple sources of the population dataset into one.
            # For now, replace the multiple sources of the population dataset by one.
            for population_source_name in ["Gapminder", "UN", "HYDE"]:
                if population_source_name in source_name:
                    source_name = "Population data is generated by Our World in Data based on different sources (https://ourworldindata.org/population-sources)"
            # TODO: Remove once we adopt the new metadata structure.
            # Also, some source names end in a period. Remove it.
            if source_name[-1] == ".":
