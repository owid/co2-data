...
def get_continents(dataframe):
    continents = pd.read_csv(
        os.path.join(INPUT_DIR, "shared/continents.csv"), usecols=["Country", "Region"]
    )
    continents = continents.merge(dataframe, on="Country")
    continents = (
        continents.groupby(["Region", "Year"], as_index=False)
        .sum()
...
def process_cdiac():
    ...
    co2_cdiac = co2_cdiac.rename(
        columns={
            "Total CO2 emissions from fossil-fuels and cement production (thousand metric tons of C)": "CO2 emissions",
            "Nation": "Country",
        }
    )
    ...
    co2_cdiac[converted_cols] = (
        co2_cdiac[converted_cols].astype(float).mul(cdiac_co2_conversion)
    )
...
def process_gcp():
    ...
    co2_gcp = pd.melt(
        co2_gcp, id_vars=["Year"], var_name=["Country"], value_name="CO2 emissions"
    )

    # Merge with country standardised names
    co2_gcp = (
        co2_gcp.merge(country_gcp, on="Country")
        .drop(columns=["Country"])
        .rename(columns={"CountryStandardised": "Country"})
    )
    converted_cols = ["CO2 emissions"]
    co2_gcp[converted_cols] = (
        co2_gcp[converted_cols].astype(float).mul(GCP_CO2_CONVERSION)
    )
    return co2_gcp
...
def add_global_emissions(dataframe):
    ...
    global_emissions = global_emissions.rename(
        columns={"Total emissions": "CO2 emissions"}
    )
...
def add_co2_peaked(dataframe):
    peak_emissions = (
        dataframe.sort_values(["Country", "CO2 emissions"]).groupby("Country").tail(1)
    )
    peak_emissions = peak_emissions[["Country", "Year"]].rename(
    ...
    dataframe.loc[
        dataframe["Year"] == dataframe["peak_year"], "CO2_peaked"
    ] = "Highest year"
...
def main():
    ...
    combined["Annual CO2 growth (%)"] = (
        combined.groupby("Country")["CO2 emissions"].pct_change() * 100
    )
    combined["Annual CO2 growth (abs)"] = combined.groupby("Country")[
        "CO2 emissions"
    ].diff()
    ...
    consumption = (
        consumption.merge(country_gcp, on="Country")
        .drop(columns=["Country"])
        .rename(columns={"CountryStandardised": "Country"})
    )
    ...
    consumption[["Consumption emissions"]] = (
        consumption[["Consumption emissions"]].astype(float).mul(GCP_CO2_CONVERSION)
    )

    # Combine consumption-based with production-based
    combined = combined.merge(consumption, on=["Country", "Year"], how="left")
    combined["Traded emissions"] = (
        combined["Consumption emissions"] - combined["CO2 emissions"]
    )
    ...
    combined["Global cumulative"] = combined.groupby("Country")[
        "Global emissions"
    ].cumsum()
    ...
    combined["CO2 per unit energy (kgCO2 per kilowatt-hour)"] = (
        ...
        / combined["Primary energy consumption (TWh) (BP & WB)"]
    )
    ...
    combined = (
        combined.drop(columns=["CO2_peaked"])
    ...
    # Save to CSV file
    combined.to_csv(os.path.join(GRAPHER_DIR, "co2_emissions.csv"), index=False)
